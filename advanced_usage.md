...
### Example
```shell
python3 -m generative_data_prep data_prep --input_path=<PATH TO DATASET FILE> --output_path=<PATH TO OUTPUT DIRECTORY> --pretrained_tokenizer=openai-community/gpt2 --max_seq_length=1024
```
...
| `pretrained_tokenizer` | str | None | Valid tokenizer key from Huggingface | The pretrained tokenizer to be used, loaded using transformers.AutoTokenizer.from_pretrained(args.pretrained_tokenizer) |
...
| `max_seq_length` | int | 2048 | Maximum seuqnece length of model you are using | The maximum sequence length of the model you are using. Typically found in model checkpoint `config.json` under `max_position_embeddings` or `max_seq_length` |
