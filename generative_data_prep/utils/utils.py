...
def data_prep_arg_builder(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The path to the output directory if using end to end data preparation, the path to output hdf5 file if running tokenization",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        required=False,
        help=f"pre-specified tokenizer class to run, defaults to gpt2, must be a choice from {list(TOKENIZER_CLASSES.keys())}",
    )
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="The pretrained tokenizer to be used, loaded using transformers.AutoTokenizer.from_pretrained(args.pretrained_tokenizer), in lieu of a custom vocab and merges file.",
    )
    parser.add_argument(
        "--vocab_file",
        default=None,
        type=str,
        required=False,
        help="The vocabulary file for the tokenizer. Should be a .json file for the tokenizer class specified by --tokenizer_class.",
    )
    parser.add_argument(
        "--merges_file",
        type=str,
    ...
    parser.add_argument(
        ...
        required=False,
        help="How to define the boundary when packing jsonl into sequences. Choosing jsonl will define each jsonl as a packing unit, and keep it together. Choosing prompt_completion_pair option, defines prompt_completion_pairs as the packing unit and will keep them together, but prompt completion pairs within one jsonl may be split into multiple sequences.",
    )
    parser.add_argument(
        "--attention_boundary",
        type=str,
        ...
        choices=BoundaryType.as_list(),
        required=False,
        help="What boundary to use when training with --article_attention flag. If you choose prompt_completion_pair tokens will only attend to tokens in the prompt_completion_pair. If you choose jsonl, then tokens will attend to all the prompt completion pairs in the jsonl",
    )
    parser.add_argument(
        "--special_tokens_dict",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="If you include this flag, then the 'category' field from your input jsonls will be stored in the 'category_id' dataset in your output hdf5 files. This flag must point to the file path of a json file that contains a list of all the strings of the 'category' keys in your dataset.",
    )
