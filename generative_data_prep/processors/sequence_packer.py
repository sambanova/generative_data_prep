...
from generative_data_prep.utils import PackingStyleType


class SequencePacker:
    """Takes articles and packs them into fixed length sequences."""
    def __init__(self, max_seq_length: int, eos_token_id: int, packing_style: PackingStyleType = PackingStyleType.FULL):
        ...
        Example:
            ...
            ... ]
            >>> for packing_style in PackingStyleType.as_list():
            ...     sequence_packer = SequencePacker(3, -1, packing_style)
            ...     sequences = sequence_packer(articles)
            ...     sequences += sequence_packer(None)
            ...     sequences_str = '\\n'.join(map(str, sequences))
            ...     print(f'Packing Style {packing_style}\\n{sequences_str}')
            ...
            Packing Style full
            ...
            Packing Style single_truncate_overflow
            ...
            Packing Style single_drop_overflow
            [(0, <TokenTypeIds.PROMPT: 0>) (1, <TokenTypeIds.PROMPT: 0>) (-1, <TokenTypeIds.PADDING: 2>)]
            [(2, <TokenTypeIds.PROMPT: 0>) (3, <TokenTypeIds.PROMPT: 0>) (-1, <TokenTypeIds.PADDING: 2>)]
            Packing Style greedy
            [(0, <TokenTypeIds.PROMPT: 0>) (1, <TokenTypeIds.PROMPT: 0>) (-1, <TokenTypeIds.PADDING: 2>)]
            [(2, <TokenTypeIds.PROMPT: 0>) (3, <TokenTypeIds.PROMPT: 0>) (-1, <TokenTypeIds.PADDING: 2>)]
        """
    ...
    def _get_packed_sequences(self, tokenized_article: TokenizedArticle, unfinished_sequence: TokenizedSequence
        ...
        if self.packing_style == PackingStyleType.SINGLE_DROP_OVERFLOW:
            ...
            # Only add the sequence if it fits, otherwise throw out the sequence.
            if remainder_article.is_empty():
                unfinished_sequence.pad()
        ...
        elif self.packing_style == PackingStyleType.SINGLE_TRUNCATE_OVERFLOW:
            # Put each article in its own sequence, fitting as much of the article as possible
            assert unfinished_sequence.is_empty()
            remainder_article = unfinished_sequence.pack(tokenized_article)
            unfinished_sequence.pad()
            newly_packed_sequences.append(unfinished_sequence)
        ...
        elif self.packing_style == PackingStyleType.GREEDY:
            ...
            if len(unfinished_sequence) + len(tokenized_article) <= self.max_seq_length:
                ...
                unfinished_sequence.pad()
                newly_packed_sequences.append(unfinished_sequence)
                # if the article fits into a sequence, create a new sequence with the article
                if len(tokenized_article) <= self.max_seq_length:
                    unfinished_sequence = TokenizedSequence.from_article(tokenized_article, self.max_seq_length,
                                                                         self.eos_token_id)
                # otherwise throw out the article
                else:
                    unfinished_sequence = TokenizedSequence.get_empty(self.max_seq_length, self.eos_token_id)

        elif self.packing_style == PackingStyleType.FULL:
            ...
            raise ValueError(f'Invalid packing style {self.packing_style}')
