...
class ArticleTokenizer:
    ...
    def process_jsonl_with_chat_template(self, jsonl: List[dict]) -> List[dict]:
        ...
        for i in range(len(split_chat)):
            if split_chat[i] == DEFAULT_PROMPT_PLACEHOLDER or split_chat[i] == DEFAULT_COMPLETION_PLACEHOLDER:
                continue
            if i > 0 and split_chat[i-1] != DEFAULT_PROMPT_PLACEHOLDER and split_chat[i-1] != DEFAULT_COMPLETION_PLACEHOLDER:
