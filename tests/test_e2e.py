...
@pytest.mark.parametrize(
    ...
    [('data_prep_test', TOKENIZER, 1024, PackingConfig.get_default(),
     ...
     ('pretraining', TOKENIZER, 1024, PackingConfig.get_default(),
      BoundaryType.JSONL, BoundaryType.JSONL),
     ('generative_tuning', TOKENIZER, 1024,
      PackingConfig.from_str('single::drop'), BoundaryType.JSONL,
      BoundaryType.JSONL),
     ('dialogue', TOKENIZER, 1024,
      PackingConfig.from_str('single::truncate_right'), BoundaryType.JSONL,
      BoundaryType.JSONL),
     ('metaICL', TOKENIZER, 1024, PackingConfig.from_str('greedy::drop'),
      BoundaryType.PROMPT_COMPLETION_PAIR, BoundaryType.JSONL)])
def test_data_prep(tokenizer: PreTrainedTokenizerBase, test_name: str,
...
@pytest.mark.parametrize(
    ...
    [('pipeline_test', False, False, 'prompt', 'completion', 'False', True,
      1024, PackingConfig.get_default(), BoundaryType.JSONL,
      BoundaryType.JSONL, None, None, None, 0.2, 0.1),
     ('pretraining_txt', False, False, 'prompt', 'completion', 'False', False,
      1024, PackingConfig.get_default(), BoundaryType.JSONL,
      BoundaryType.JSONL, 32, 0, 0, None, None),
     ('pretraining', False, False, 'prompt', 'completion', 'False', False,
      1024, PackingConfig.get_default(), BoundaryType.JSONL,
      BoundaryType.JSONL, 32, 0, 0, None, None),
     ('generative_tuning', False, False, 'prompt', 'completion', 'False',
      False, 1024, PackingConfig.from_str('single::drop'), BoundaryType.JSONL,
      BoundaryType.JSONL, 32, 0, 0, None, None),
     ('dialogue', False, False, 'prompt', 'completion', 'False', False, 1024,
      PackingConfig.from_str('single::truncate_right'), BoundaryType.JSONL,
      BoundaryType.JSONL, 32, 0, 0, None, None),
     ('metaICL', False, False, 'prompt', 'completion', 'False', False, 1024,
      PackingConfig.from_str('greedy::drop'),
