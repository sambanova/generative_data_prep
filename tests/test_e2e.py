...
@pytest.mark.parametrize('tokenizer,input_name,max_seq_length,input_packing_style,packing_boundary,\
                         attention_boundary,gold_name',
                         [(TOKENIZER, 'data_prep_ex.jsonl', 1024, PackingStyleType.FULL,
                           BoundaryType.JSONL, BoundaryType.JSONL, 'data_prep_ex.hdf5')])
def test_data_prep(tokenizer: PreTrainedTokenizerBase, input_path: str, max_seq_length: int,
    ...
    with tempfile.TemporaryDirectory() as output_dir:
        ...
        data_prep_main(silent=True,
                       ...
                       input_packing_style=input_packing_style,
...
@pytest.mark.parametrize(
    'input_name,gold_name,disable_space_separator,prompt_keyword,completion_keyword,shuffle,do_not_balance_hdf5,\
        max_seq_length,input_packing_style,packing_boundary,attention_boundary,\
            num_training_splits,num_dev_splits,num_test_splits,dev_ratio,test_ratio',
    [('pipeline_ex_data.jsonl', 'pipeline_no_balanced', False, 'prompt', 'completion', 'on_RAM', True, 1024,
      PackingStyleType.FULL, BoundaryType.JSONL, BoundaryType.JSONL, None, None, None, 0.2, 0.1),
     ('pipeline_ex_data.jsonl', 'pipeline_balanced', False, 'prompt', 'completion', 'on_RAM', False, 1024,
      PackingStyleType.FULL, BoundaryType.JSONL, BoundaryType.JSONL, 20, 5, 0, None, None)])
def test_pipeline(input_path: str, gold_path: str, disable_space_separator: bool, prompt_keyword: str,
                  completion_keyword: str, shuffle: str, do_not_balance_hdf5: bool, max_seq_length: int,
                  input_packing_style: PackingStyleType, packing_boundary: BoundaryType,
                  attention_boundary: BoundaryType, num_training_splits: int, num_dev_splits: int,
                  num_test_splits: int, dev_ratio: float, test_ratio: float):
